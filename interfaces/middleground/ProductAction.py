...
class ProductAction(object):
    ...
    def _admin_release_page_opt_list(self, pn_=None, ps_=None, channelId_=None, labelId_=None, labelName_=None, skuName_=None):
        if self.user is None:
        ...
        response = self.request.post(url=self.url+'/admin/release/page-opt-list', data=data, hosts=self.url)
        return self.__judge_response_status(json.loads(response))
